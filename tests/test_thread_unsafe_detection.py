...
def test_thread_unsafe_fixtures(pytester):
    ...
    pytester.makepyfile("""
        ...
        def test_capteesys(capteesys, num_parallel_threads):
            assert num_parallel_threads == 1

        def test_capsysbinary(capsysbinary, num_parallel_threads):
            assert num_parallel_threads == 1

        def test_capfd(capfd, num_parallel_threads):
            assert num_parallel_threads == 1

        def test_capfdbinary(capfdbinary, num_parallel_threads):
            assert num_parallel_threads == 1
    ...
    result.stdout.fnmatch_lines(
        [
            ...
            "*::test_capteesys PASSED *thread-unsafe*: uses thread-unsafe fixture*",
            "*::test_capsysbinary PASSED *thread-unsafe*: uses thread-unsafe fixture*",
            "*::test_capfd PASSED *thread-unsafe*: uses thread-unsafe fixture*",
            "*::test_capfdbinary PASSED *thread-unsafe*: uses thread-unsafe fixture*",
